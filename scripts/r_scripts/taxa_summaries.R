...
Order_sumry <- read_dat %>% 
  group_by(Order) %>% 
  summarise(Family=n_distinct(Family),
            Genus=n_distinct(Genus),
            Species=n_distinct(Species),
            ASV=n_distinct(ASVid)) %>% 
  arrange(-ASV)

## plot table; save as 'Order_summary_allSites'; export at 375x470
formattable(Order_sumry, list(
  ASV = color_tile("white", "dodgerblue"),
  Species = color_tile("white", "green"),
  Genus = color_tile("white", "orange")
))
...
genus_sumry <- as.data.frame(read_dat %>% 
  mutate(YearLabel=paste0(StudyID, Window)) %>% 
  group_by(Order, Family, Genus) %>% 
  summarise(ASVs=n_distinct(ASVid),
            Sites=n_distinct(Site),
            CollectionWindows=n_distinct(YearLabel),
            Samples=n_distinct(SampleID)) %>% 
  arrange(-ASVs))

## write full table to disk:
write.csv(genus_sumry, file="~/Repos/nhguano/data/diet_summary_tables/genus_summary.csv",
          row.names = FALSE, quote = FALSE)

## filter just those instances where a particular Genera are observed in at least 15 sites (of 20 total)
genus_fmtbl <- formattable(genus_sumry %>% filter(Sites >= 15) %>% arrange(-Sites, -Samples), 
                           list(ASVs = color_tile("white", "dodgerblue"),
                                Samples = color_tile("white", "red")
))
...
## export here; save as 'Genus_summary_allSites'
setwd("~/Repos/nhguano/figures/")
export_formattable(genus_fmtbl,"Genus_summary_allSites.png")
