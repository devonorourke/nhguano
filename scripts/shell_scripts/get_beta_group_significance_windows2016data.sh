...
for PREFIX in $(printf "win456\nwin567\n"); do
  ...
  ## export each beta group significance visualization, collect raw text data, and add
    for METRIC in $(printf "dice\nbray\nuuni\nwuni\n"); do
      ...
      awk -v var=$PREFIX '{print $0"\t"var}' > "$PREFIX"_betasig_site_"$METRIC"_data.tsv
    done

    ## combine all outputs into single file for particular window group:
  printf 'SubjectID1\tSubjectID2\tGroup1\tGroup2\tDistance\tMetric\tWindowGroup\n' > "$PREFIX"_betasig_site_allMetrics_data.tsv
